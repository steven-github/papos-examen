...
export default function MockExamPage() {
    ...
    const score = useMemo(() => Math.round((correctCount / questionSet.length) * 100), [correctCount, questionSet.length]);
    ...
    return (
        <div className='page-shell'>
            ...
            <main className='content-wrap mx-auto flex w-full max-w-7xl  flex-col gap-6 px-4 py-6 sm:px-6'>
                <ChildHeader
                    eyebrow='Simulacro'
                    title='Simulacro de Grammar'
                    subtitle='Repaso final de Grammar. Lee con calma y responde paso a paso.'
                        ...
                        {currentIndex > 0 || answeredCurrent ? (
                            <div className='glass-card rounded-4xl p-5'>
                                {answeredCurrent && pendingFinalScore === null ? (
                                    <p className='text-sm font-bold text-slate-600'>Lee la explicacion con calma. Avanza cuando ya la entiendas.</p>
                                ) : null}
                                {answeredCurrent && pendingFinalScore !== null ? (
