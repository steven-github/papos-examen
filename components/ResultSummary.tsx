...
export function ResultSummary({ score, correct, total, onRetry, title }: ResultSummaryProps) {
  return (
    <div className="glass-card rounded-4xl p-6">
      ...
      <h3 className="section-title text-3xl text-slate-800">{title ?? "Nice work!"}</h3>
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="rounded-3xl bg-white/90 p-4">
          <div className="mb-2 flex items-center gap-2 font-black text-emerald-700">
            <CheckCircle2 className="h-5 w-5" /> What went well
          </div>
          <p className="text-sm font-bold text-slate-600">You answered {correct} questions the right way.</p>
        </div>
        <div className="rounded-3xl bg-white/90 p-4">
          <div className="mb-2 flex items-center gap-2 font-black text-rose-700">
            <XCircle className="h-5 w-5" /> What to review
          </div>
          <p className="text-sm font-bold text-slate-600">Check the explanations for any tricky questions and try again.</p>
        </div>
      </div>
      {onRetry ? (
        <button
