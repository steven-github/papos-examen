    ...
    >
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="section-title text-2xl text-slate-800">{lesson.title}</h3>
        </div>
      ...
      <p className="mb-6 text-sm font-semibold text-slate-700">{lesson.simpleExplanation}</p>
      {unlocked ? (
        <Link
        ...
        <div className="inline-flex items-center gap-2 rounded-full bg-slate-900/10 px-4 py-3 text-sm font-black text-slate-500">
          <Lock className="h-4 w-4" /> Finish the lesson before this one
        </div>
      )}
    </article>
  );
